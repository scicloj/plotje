...
- **Every log axis whose domain does not reach its outermost breaks.** A tick outside the domain is no longer drawn: on a log y axis over 3 to 900 the generated breaks were 1, 10, 100 and 1000, and the 1 was labelled below the panel altogether. ggplot2 drops the same break on the same domain (measured, 4.0.0). An axis that would be left with fewer than two labels keeps them all, since a narrow domain such as 6 to 9 has only one break to offer and no labels at all is worse than one just outside.
- **Every numeric axis whose values are whole numbers, over a short span.** The ticks are whole too: a column of 0, 1, 2 and 3 is ticked 0, 1, 2, 3, where it was ticked 0.0, 0.5, 1.0 and so on to 3.0. The rule reads the values the axis draws and the extent they cover, not the domain, which has been padded by the time ticks are chosen; an axis with a value between two whole numbers, a `:domain` written with a fraction, or a `:position :fill` that rewrites counts as proportions is ticked as before. Thirty-nine axes in the documentation move, all of them counts or small whole ranges.
- **Every log axis spanning a few decades.** Ticks are chosen by how many of them the axis draws rather than how many the generator offers, so an axis whose powers of ten mostly fall outside its domain carries 1-2-5 or 1-2-3-5 breaks between them. Gapminder's gross domestic product per capita, from 214 to 63951, carried 1000 and 10000 at every width; it now carries seven labels from 500 to 50000 at the default width and ten at 620 wide. A domain holding three or more powers of ten inside it is ticked at them as before.
- **Every continuous legend on a log scale.** The gradient bar is labelled only at the ticks that fall on it; a tick above the top of the bar used to be drawn over the legend's title. Where fewer than two ticks fall inside, the bar carries its two end values, which is what a linear bar carries.
- **Every stacked bar, area and step drawn from a zero baseline.** The baseline sits on the edge of the panel, where an unstacked bar's already sat; it used to be padded past, so a stacked chart and an unstacked one of the same column drew their baselines in different places. Two stacks of 30 and 70 span 0 to 73.5, where they spanned -3.5 to 73.5.
- **Every date axis whose ticks step by whole days and named no month.** The ticks are labelled with their month as well as their day -- 28 daily dates in January give `Jan-02`, `Jan-05`, `Jan-08` in place of `02`, `05`, `08`, and a ten-day span gives the same in place of `Tue 01:00`, whose weekday comes round again once the span passes a week. An axis already naming a month or a year is unchanged, as is one whose ticks step by hours, where the hour is what tells them apart. ggplot2 labels the same 28 days with a month too.
...
- **A `:domain` on a categorical `:color`.** The order it gives now reaches the marks as well as the legend, so the segments of a stacked bar and the members of a dodged group follow the rows of the legend beside them -- bars, boxplots and violins alike. Before, the domain moved the legend and left the marks in whatever order the stat had grouped them.
- **Every horizontal dodged chart.** The groups within a band run from the top down, in the order the legend lists them, where they used to run from the bottom up. Bars, boxplots, violins, error bars and point ranges are placed the same way, and a vertical chart is unchanged -- its groups run left to right, as before. ggplot2 runs a horizontal dodge from the bottom up (measured, 4.0.0); Plotje matches its legend instead.
- **Every stacked or dodged step.** `pj/lay-step` reads `:position`, where the layer used to reach the position stage carrying none and draw its raw values. The chapter's own Stacked Step example was three unstacked lines.
- **Every faceted plot with a `:size`, `:alpha` or numeric `:color` column, and every plot with two such layers.** The aesthetic is read against every value the plot holds rather than against one panel's or one layer's, so a value means one size, one opacity and one colour wherever it is drawn, and the legend explains all of them. A facet whose panels cover different intervals now draws them at different sizes, which is what comparing panels needs. ggplot2 keeps one scale across facets and layers too (measured, 4.0.0).
...
- `:include` on an `:x` or `:y` scale names a value the axis has to reach. `(pj/scale pose :y {:include 0})` puts zero on the axis, so that lengths drawn along it are proportional to the values -- ggplot2's `expand_limits(y = 0)` -- and the value lands exactly at the edge of the panel, where `pj/lay-bar` already draws its baseline. A collection names more than one, so `{:include [0 100]}` gives an axis room the data does not fill; it is a set, and `[100 0]` says the same thing, where a `:domain` is an ordered pair that draws the axis the other way up when it is written the other way round. A `:domain` and an `:include` on one aesthetic are reported, as is an `:include` on a log axis, which has no reading for zero, or on a categorical one, which has no extent between its categories.
- `pj/valid-pose?` and `pj/explain-pose` check a pose against its schema, the pair `pj/valid-plan?` and `pj/valid-membrane?` already gave the two later stages. The schema is structural: it says what shape a pose has, not whether the columns it names are in the data. A pose built by `pj/pose`, `pj/lay-*`, `pj/options`, `pj/facet`, `pj/arrange`, `pj/coord` or `pj/scale` conforms by construction, so the pair is for a pose that has been reached into and changed.

### Reported rather than passed over

- An axis `:domain` is checked against the column it is read for. Against a continuous column it is two finite numbers: `{:domain [0]}` drew an axis with a single tick and the marks off the panel, and `{:domain [0 nil]}` died in the arithmetic, naming neither the axis nor the setting. Against a categorical column it is still the list of categories that orders them, and the check is made where the column's type is known, since that type is what decides which of the two a domain is.
- `:x-type`, `:y-type` and `:color-type` report a type the column cannot be read as. Only `:categorical` can be asked of a column holding something else, because every value can name a group; `:numerical` and `:temporal` are accepted where they name the column's own type and reported where they do not. A value that is no column type at all is reported too, where it used to be taken as `:numerical`. The message names the option, the column and what the column holds, in place of a `ClassCastException` from inside the scale.
- `pj/to-drawing` and `pj/to-data` report a pose, a plan or the whole frames map written in the panel's place. A panel entry is one element of `(:panels (pj/frames plot))`, and each message names which of the three it was given and the call that reaches a panel entry from there. Both arities of both functions report it, as do `nil` and anything else that is not a panel entry.
- A row dropped for a missing value on a non-numeric column says so, as a row dropped for a missing number already did. A `nil` among the values of a string or keyword column leaves its row with no category to belong to, and the message names the column: `Removed 1 rows with a missing category in :species`. A column holding no values at all -- an all-`nil` `:size`, or a `:y` of `NaN`, which is typed as boolean rather than numeric -- reports a missing *value* rather than a category. Marks that keep such rows rather than dropping them stay silent, so the report and the drop always agree.

### Fixed
- a temporal axis takes a `:domain` written as dates, as its `:breaks` do. The axis holds epoch milliseconds, and a domain of `LocalDate` values used to reach the marks unconverted, where the first mark placed died on `Number.doubleValue() because "x" is null`.
- a date axis takes `:breaks` written as dates. `(pj/scale :x {:breaks [(jt/local-date 1940 1 1) ...]})` places a tick at each of them, and `:tick-labels` relabels them; a `LocalDate`, a `LocalDateTime` and a `java.util.Date` are all accepted. The axis holds epoch milliseconds, and the dates used to reach the out-of-range check unconverted, where they failed with `LocalDate cannot be cast to Number`.
- a temporal axis with a single tick is labelled. wadogo's datetime formatter reduces `min` over the gaps between ticks, so one tick left it with none.
- a stat that needs a numeric column reports one it cannot read, whatever the column is named. The message used to be built with `name`, which throws on the integer column names a dataset built without them gets, so `pj/lay-histogram`, `pj/lay-density`, `pj/lay-smooth`, `pj/lay-tile` and `pj/lay-contour` answered with a `ClassCastException` instead. The message also names a way forward: dates written as text stay categorical until they are parsed.
- a mark that groups by category no longer calls an empty column numerical. An empty column and one holding nothing but `nil` are both typed boolean, and both were reported as `is numerical`, which sent the reader to check a column type that was not the problem.
- `:text` given a value it cannot read says which value it was. The message used to tell the reader to pass `:text` as an option, which is what they had done.
- rows written as vectors are read as data: `(pj/lay-point [[1 2] [3 4]])` draws two points, with columns 0 and 1 inferred as the `:x` and `:y` aesthetics. `membrane.ui` treats a vector as a group of drawables, so the check that catches a rendered plot piped back into a pose function answered this with a message about Membrane elements. `pj/pose` accepted the same shape throughout.
- a layer that names no scale has no opinion about it on every aesthetic, as it always has on an axis, and is drawn against the one scale the plot settles on. `:size`, `:alpha`, `:color` and `:fill` used to count an unnamed scale as naming the default, so a `:log` written on one layer was refused for disagreeing with the silence of the next. Two layers that each name a different scale are still refused.
- a size or alpha legend labels only values the data reaches. On a narrow log domain every tick the generator picked could land outside it -- values from 6 to 9 gave the single entry 10 -- so the legend paired a swatch with a value no mark carried.
