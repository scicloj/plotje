...
- An axis whose domain is shared across the cells of a composite is padded as an unshared axis is, so the outermost marks stand inside the panel rather than on its edge. `:share-scales` computes an extent across the cells, and that extent was written as a `:domain` -- which the plan honours exactly as given, correctly for a domain the writer wrote and not for one the cells were given. Every scatterplot matrix, every `pj/marginal`, and every composite carrying `:share-scales` gains the padding. A `:domain` written with `pj/scale` is unchanged.
