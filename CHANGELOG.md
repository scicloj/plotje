...
## [Unreleased]
- fix: a plot draws the same numbers whatever machine renders it. Tick labels, legend endpoints and tooltip numbers were formatted through `clojure.core/format`, which reads the JVM's default locale, so an axis that read `1.2` here read `1,2` on a German JVM -- and with `:thousands-separator ","` set, one plot drew a comma meaning a decimal point on one axis and a thousands separator on the other. All of it is formatted under `Locale/ROOT` now, as SVG coordinates have been since 0.2.2.
- new configuration key `:decimal-separator` draws the decimal point as whatever string you name, in the same text `:thousands-separator` groups: `(pj/options {:thousands-separator "." :decimal-separator ","})` writes 1.234,5. Off by default, and chosen here rather than inherited from the machine.
- fix: `pj/save` honors every plot option `pj/plot` honors. It rebuilt the render stage's options from a fixed list that five keys were missing from, so `{:thousands-separator ","}` grouped an axis and not the legend beside it, and `{:title-font-size 30}` was written at 15. `:label-font-size`, `:grid-stroke-width` and `:annotation-stroke` were the others. Both SVG and PNG.
- fix: a composite's cells read the composite's options. `pj/arrange` rendered each cell with no options at all, so a plot option that moved a single plot did nothing once the same pose was arranged. A cell still decides its own `:tooltip`.
- strip labels read `:strip-font-size` wherever they are drawn, and the default is 11 where it was 10. A grid composite's strip labels took a hardcoded 11 and a facet's took the configured 10, so two kinds of strip in one book were a point apart and only one of them answered the option. Facet strips are now a point larger.
- fix: a composite's title reads `:title-font-size`, and its shared legend reads the configuration the cells read -- `:thousands-separator` among it, where a shared legend read 100000 beside axes reading 100,000. The title took the literal 15 that key defaults to.
