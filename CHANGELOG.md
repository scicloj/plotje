...
- new function `pj/frames` publishes where a plot's panels sit on the canvas: per panel, the `:canvas`, `:panel-box` and `:drawing-area` rectangles, in drawing units. It is the same computation the renderer draws with, and a composite reports every cell in canvas coordinates. The result is data all the way down, so it prints and round-trips. - thanks, @timothypratley
