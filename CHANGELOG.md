...
An axis is ticked for the values it carries. A column of whole numbers gets whole ticks, a log axis keeps its ticks inside its own panel and is labelled between its decades, and a date axis is ticked on round calendar steps, labelled at the unit those steps take, and takes its `:breaks` and its `:domain` written as dates. A size or alpha legend reads the same rule as the axis beside it, so a column mapped to both is labelled one way in both -- on a linear scale and on a log one.
...
The retired name reports the one to write. Every published docstring and arglist says *aesthetic* where it said *channel* -- `pj/scale` takes `[pose aesthetic scale-type]` -- and the errors `layer-type/register!` throws about `:varies` carry `:aesthetic` in their data.
...
- **Every log axis whose domain does not reach its outermost breaks.** A tick outside the domain is not drawn, as ggplot2 drops it on the same domain. A domain too narrow for two breaks is ticked across itself instead: 6 to 9 is ticked 6, 7, 8, 9, where it drew its only break, 10, above the top of the panel, and 7 to 8 drew no ticks at all. ggplot2 answers such a domain the same way round, by choosing breaks inside the range rather than by filtering.
...
- **Every continuous legend on a log scale.** The gradient bar is labelled only at the ticks that fall on it, and a domain too narrow for two breaks is ticked across itself as an axis is. A size legend, an alpha legend, a gradient bar and an axis read one rule for a log scale, so a column mapped to two of them is labelled the same way in both.
- **Every date axis.** Its ticks step by a round number of years, months, weeks, days, hours, minutes, seconds or milliseconds, and each one lands on a multiple of that step counted from the calendar rather than from the first value. A span of 1938 to 1971 is ticked 1940, 1945, 1950 in place of 1939, 1943, 1947; a year of monthly readings begins at January in place of February; a weekly axis lands on Mondays. ggplot2 places its date breaks the same way.
- **Every date axis whose ticks step by a whole year or a whole month.** The label carries the unit the ticks step by and nothing finer: fifteen years of January firsts are ticked `2002`, `2004`, `2006` where they read `2002-01`, `2003-01`, `2004-01`, and a year of month starts is ticked `Jan 2020`, `Mar 2020`, `May 2020` where it read `Feb-01`, `Mar-01`, `Apr-01`. The day was the same on every one of those ticks, so it took up room without telling any two apart, and no part of the picture said which year it was. A month-stepped axis carries its year once it covers about one, and an axis of a few months carries the month alone. ggplot2 labels both spans the same way, turning over between a 301-day panel and a 368-day one.
- **Every date axis whose ticks step by whole days and named no month.** The ticks carry their month as well as their day: `Jan-02`, `Jan-05`, `Jan-08` in place of `02`, `05`, `08`. An axis whose days run across two years names the year in their place, and one whose ticks step by hours is unchanged, where the hour is what tells them apart. ggplot2 labels the same days with a month too.
...
- **Every stacked bar, area and step.** The groups pile up in the order the legend lists them, the first on top, and `:position :fill` stacks the same way round. ggplot2's `position_stack` stacks this way too, so a reader matches the legend against the bar by reading both downward.
...
- **Every faceted plot with a `:size`, `:alpha` or numeric `:color` column, and every plot with two such layers.** The aesthetic is read against every value the plot holds rather than against one panel's or one layer's, so a value means one size, one opacity and one colour wherever it is drawn, and the legend explains all of them. ggplot2 keeps one scale across facets and layers too.
...
- An axis `:domain` is checked against the column it is read for. Against a continuous column it is two finite numbers, and against a categorical one it is the list of categories that orders them. The check is made where the column's type is known, since the type is what decides which of the two a domain is.
- `:x-type`, `:y-type` and `:color-type` report a type the column cannot be read as. Only `:categorical` can be asked of a column holding something else, because every value can name a group; `:numerical` and `:temporal` are accepted where they name the column's own type. The message names the option, the column and what the column holds.
...
- A mark that draws one colour per group reports a numeric `:color` column, which it cannot read as a gradient -- bars, boxplots, violins, lollipops, lines, steps, areas, histograms, densities, smooths, rugs, text and labels among them. `:point` is the mark that carries a colour per row, and a contour reads the `:color` scale its levels are built from. The message names `:color-type :categorical`, which gives each category a colour of its own.

### Fixed

- A mark drawn from a baseline rests on a log value axis instead of being drawn from an infinity. A log scale has no reading for zero, and `pj/lay-area` and `pj/lay-lollipop` closed to zero whatever the axis was, so an area on a log axis drew a bowtie and a lollipop lost every stem -- on data with no zero in it. The baseline is now the panel's own smallest value, which is where `pj/lay-bar` already put it.
...
- A `ZonedDateTime` and an `OffsetDateTime` column is read as a date. Each carries its own offset, so each names an instant, and both are read at UTC as the other temporal types are. A `ZonedDateTime` was typed temporal by the dataset and then refused by the tick generator; an `OffsetDateTime` was drawn as one tick per value without a word.
- A column of values naming a stretch of time rather than a moment is reported: a `YearMonth`, a `Year`, a `MonthDay`. Grouping by month is what such a column comes from, and it was drawn with one tick per value -- a hundred and eighty over fifteen years -- in place of a calendar axis. The message names what the column holds and the line that maps it to the day it starts. Which instant a month stands for is left to the writer.
...
- A date axis spanning under two seconds is ticked on round fractions of a second, and each tick is labelled to the precision its own step needs. Milliseconds were the one unit the step list did not reach, so such a span fell back to dividing itself by the tick count: half a second of data was ticked a millisecond after the first value and every 62 ms after that, and the labels, fixed at tenths, read 0, 0, 1, 1, 2, 3, 3, 4, 4 -- nine ticks and five labels. It now reads 00.0 through 00.5.
- A temporal axis with a single tick is labelled, and the tick is drawn where the value sits. A domain of one date is padded like any other -- by a fraction of the epoch-millisecond number, which is years wide -- so the tick was drawn at the edge of the panel carrying a date two and a half years to its right, while the mark sat in the middle.
...
- A log `:fill` legend spans the values the tiles carry. A two-dimensional bin and a density grid both report a range measured from none, which is what a linear count legend should span; on a log scale, where none has no reading, the low end is read from the tiles instead. Read from the floor, the generator offered every decade from 1e-300 and the bar carried three hundred labels printed over each other. A fill that reaches zero even so -- a `:domain` written from it, or a density that underflows -- is reported.
