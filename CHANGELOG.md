...
- `pj/marginal` draws a distribution of one of a pose's columns in a thin panel against one edge of the plot: `(-> pose (pj/marginal :top))` puts a density of the `:x` column above, and `(-> pose (pj/marginal :right))` puts one of the `:y` column beside it, drawn on its side. `:histogram` draws bars instead of a curve, and `:size` sets the strip's share of the height or width. The strip shares the main panel's axis for that column and keeps its own value scale, its duplicate ticks and axis title are dropped, and the two drawing areas are aligned so a value sits at the same place in both -- including when the main panel carries a legend. `:bottom` and `:left` are reported: a strip there would sit between the panel and the axis that describes it. A categorical column is reported too, since a density and a histogram both read numbers.
...
- `:tooltip` is an aesthetic: map a column to it and its values are what each mark says on hover, or map a string and every mark of the layer says that. What a tooltip reads is then ordinary data work -- a currency sign, a thousands separator, 1,653,346 read as `1.7M`, a percentage, a column the plot does not draw -- rather than a formatting option per case. It scopes like any mapping, the nearer one winning, and writing it turns tooltips on, so `{:tooltip true}` in `pj/options` is the switch for the built-in text and is not needed beside a mapping. A newline in the string breaks the line, and a column of hiccup gives a tooltip with markup in it -- `[:div [:b month] [:br] "revenue " [:code amount]]`. A string tooltip stays text, so a string that spells out a tag is shown as that text rather than rendered. - thanks, @otfrom
...
- An axis whose domain is shared across the cells of a composite is padded as an unshared axis is, so the outermost marks stand inside the panel rather than on its edge. `:share-scales` computes an extent across the cells, and that extent was written as a `:domain` -- which the plan honours exactly as given, correctly for a domain the writer wrote and not for one the cells were given. Every scatterplot matrix, every `pj/marginal`, and every composite carrying `:share-scales` gains the padding, and bars in such a composite gain headroom: a shared extent of 0 to 9 is drawn against 0 to 9.45, so the tallest bar no longer touches the top of the panel. The baseline stays on the panel edge, and a `:domain` written with `pj/scale` is unchanged.

- `:share-scales` reads an axis only one cell names as needing no work, rather than refusing it. Sharing is between cells, and a cell that shares with nobody is drawn against its own extent either way, so refusing those reported an axis nothing was asking to share. A right-hand `pj/marginal` on a plot whose other axis holds dates is drawn rather than reported, and where a column genuinely cannot be shared the message still names it.

- A histogram under `(pj/coord :flip)` draws its bars across the panel, measured from the count axis. A bin edge belongs on the axis the bins run along and a count on the other, and the flip swaps which is which; both were read from the upright pair, so every bar of a flipped histogram was drawn within the first few drawing units of the panel.
