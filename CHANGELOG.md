...
- A stat that needs a numeric column reports one it cannot read, whatever the column is named, and names a way forward: dates written as text stay categorical until they are parsed. `pj/lay-histogram`, `pj/lay-density`, `pj/lay-smooth`, `pj/lay-tile` and `pj/lay-contour` all report it.
