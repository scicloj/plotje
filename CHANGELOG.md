...
- **Every log axis whose domain does not reach its outermost breaks.** A tick outside the domain is not drawn, as ggplot2 drops it on the same domain. An axis that would be left with fewer than two labels keeps them all: a narrow domain such as 6 to 9 has one break to offer, and no labels at all is worse than one just outside.
...
- **Every continuous legend on a log scale.** The gradient bar is labelled only at the ticks that fall on it. Where fewer than two fall inside, it carries its two end values, which is what a linear bar carries.
