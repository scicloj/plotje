...
- a position may be given as a value, not only as a column: `(pj/lay-text pose {:x 6.5 :y 3.5 :text "mean"})` places a note without inventing a one-row dataset for it. Every appearance aesthetic already accepted a literal (`{:color "red"}` beside `{:color :species}`); positions were the exception. On a layer that brings no data of its own, a string `:text` is the text rather than a column name. A literal position is a data value like any other, so it takes part in the axis domains, as ggplot2's `annotate()` does. - thanks, @behrica
- new layer option `:in` names the space a layer's positions are in: `:data` (default) or `:drawing-area`, which measures in drawing units from the top left of the panel background. `(pj/lay-text pose {:in :drawing-area :x 12 :y 12 :text "n = 150"})` puts a note in the panel's corner and leaves the axis domains alone.
- new layer options `:offset-x` and `:offset-y` shift a layer by a number of drawing units after the scales run. This is what `:nudge-x`/`:nudge-y` cannot be: the amount a label needs to clear its point is a length on the page, not a data value, so no nudge is right for it across two scales -- and on a categorical axis `:nudge-x` throws, where an offset works. Universal, so every layer type takes them, and the domain widens to keep an offset label in full. - thanks, @behrica
- new function `pj/frames` publishes where a plot's panels sit on the canvas: per panel, the `:panel-box` and `:drawing-area` rectangles in drawing units, plus the `:canvas` once for the plot. It is the same computation the renderer draws with, and a composite reports every cell in canvas coordinates. The result is data all the way down, so it prints and round-trips. - thanks, @timothypratley
- new functions `pj/to-drawing` and `pj/to-data` map between the spaces for one panel of `pj/frames`, each taking a single x and y or a collection of pairs. `pj/to-data` is the direction an interaction needs -- what value is under the pointer -- and throws under `:polar`, whose projection has no coordinate-by-coordinate inverse; a panel says which it is in `:invertible?`.
- a text mark that widens its axis to be drawn in full now leaves a few drawing units of air rather than fitting exactly, so the last glyph no longer touches the panel edge.
- fix: `:group` no longer discards a literal `:color`. `(pj/lay-line data :x :y {:group :country :color "#d0d0d0"})` draws one line per country, all in that grey; before, the grouping won and every line took the palette's first color. This is what a pale supporting cast behind a few named series needs -- see the new Annotated Charts recipes in the Cookbook.
