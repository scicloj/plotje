...
(def aesthetic-registry
  "Per-aesthetic properties, and the single table the rest of the
   ...
   says on hover, and a written value says one thing for every mark of
...
(def plot-option-docs
  ...
  {:title ["Content" "Plot title string"]
   ...
   :color-label ["Content" "Color legend title, overriding the inferred column name. The outermost scope of the same setting `:label` names in a `:color` scale spec, so a spec written on a mapping or a layer wins over it. It also titles the legend of a mark drawn in a fill where `:fill-label` is not given, as the other fill settings fall back to their `:color` ones"]
   :fill-label ["Content" "Title of the legend of a mark drawn in a fill -- a tile, whether it reads `:fill` or `:color`, a 2D density or a 2D histogram -- overriding the inferred name. Read after a `:label` in a `:fill` or `:color` scale spec, and before `:color-label`, which is the order every fill setting is read in. A contour is coloured through `:color`, so `:color-label` titles its legend"]
