...
(def layer-option-docs
  ...
  {:x "Column keyword or string naming the column drawn along the x axis, or a value to draw at that x. A value beside a column :y broadcasts over the layer's data; values for both :x and :y draw one mark"
   ...
   :align-x "Horizontal text anchor -- :left, :center, or :right (default :left); which part of the label sits at its x"
   :align-y "Vertical text anchor -- :top, :center, or :bottom (default :center); which part of the label sits at its y. Data-oriented: :top puts the label's top edge at the point"
   ...
   :side "Which axis margin a rug's ticks are drawn in -- :x (default), :y, or :both"
   :density-2d-grid "2D density grid resolution -- number of bins per axis (default 25)"
   :y-intercept "Numeric or temporal y value a horizontal reference line is drawn at. Read straight from the mapping, so it names no column; {:value 2.0} says the same thing at more length"
