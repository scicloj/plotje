;; # Placing Marks
;;
;; Where a mark goes, and in what units. A scale turns a data value into
;; a place on the page, and most of the time that is the whole story. It
;; stops being the whole story when a mark has to sit a few units clear
;; of another one, when a note belongs somewhere no row of the data
;; mentions, or when you need to know where a value landed in order to
;; draw something beside it.
;;
;; Everything here except the anchor options and `:fit-text-domain`
;; applies to any layer type, not only to text. Text just runs into it
;; first, because a label is drawn at another mark's coordinates and has
;; a size of its own.
...
;; ## Two spaces
;;
;; A number that places a mark is only meaningful together with the space
;; it is measured in, and Plotje uses two.
;;
...
;;
;; Three rectangles matter in drawing space, and the
;; [Glossary](./plotje_book.glossary.html) defines all three: the
;; **canvas** is the whole image, a **panel box** is one panel including
;; its axis margin, and the **drawing area** is the shaded background
;; inside that margin, where the marks are drawn.
;;
;; Every feature in this chapter is one of two moves: give a number in
;; data space, or give one in drawing space. Sorting a placement problem
;; into the right space is most of solving it.
...
;; shift of a few units on the page, and that is not a data quantity: it
;; depends on the size the mark is drawn at, not on what the axis
;; measures.
;;
;; It is worth seeing why the data-space tool cannot do this job.
;; `:nudge-x` shifts the datum before the scales run. Six cars, labelled
;; with their names, nudged along an axis of weights in thousands of
;; pounds -- 0.08 happens to be about the width of the marker here:
...
;; to 460 rather than 1.9 to 5.4, so the same nudge moves the text by
;; about a tenth of a drawing unit and the labels sit back on their
;; markers:
...
;; points. This is what ggplot2's `annotate()` does as well -- it builds
;; a one-row data frame behind the scenes.
;;
...
;; space is a datum and trains the axis; a number in drawing space is a
...
;; the canvas -- the same computation the renderer draws with. Reach for
;; it to line a hand-placed mark up with a data one, to compose your own
;; drawing beside the plot, or to answer what value is under a pointer.
...
;; That arity maps whole columns and builds the panel's scales once, so
;; it is the one to reach for when placing many positions:
...
;; coordinates of a point share one index space -- a dataset states that,
;; where two loose sequences only promise it.
;;
;; Those numbers are canvas coordinates, measured from the top left of
;; the whole image, which is what the frames report. A
;; `{:in :drawing-area}` layer measures from the drawing area's own
;; corner, so drawing a mapped position back means subtracting that
;; corner first:
...
(let [panel     (-> scatter pj/frames :panels first)
  ...
  (pj/lay-point scatter {:in :drawing-area
                         :data (tc/dataset {:x (dfn/- (canvas :x) dax)
                                            :y (dfn/- (canvas :y) day)})
...
(kind/test-last
 [(fn [fr]
    ...
    ;; the data. Each lands on the point whose values it was mapped
    ;; from, which is the claim worth making: the published mapping and
    ;; the renderer put a value in the same place.
...
;;
;; This also settles the question the nudge section raised. How far does
;; a nudge of 0.08 move a label, on each of those two axes? Ask the two
;; panels where 0.08 of their own units lands:

(let [shift (fn [column]
              (let [panel (-> cars (pj/lay-point column :mpg) pj/frames
                              :panels first)
...
;; Eleven drawing units against a tenth of one, for the same number
;; written in the layer.
;;
...
;; A drawing-space layer is safe to add after measuring, though: it takes
;; no part in the domains and asks for no margin, so the drawing area it
;; was placed in is the one it lands in. Here the caption is put in the
;; bottom left corner, which needs the panel's height:
...
(kind/test-last
 [(fn [fr]
    (let [drawing-area #(-> % pj/frames :panels first :frames :drawing-area)]
      (and (some #{"n = 6"} (:texts (pj/svg-summary (pj/plot fr))))
